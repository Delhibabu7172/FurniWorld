...
import { getBrandApi, postBrandApi, putBrandApi, uploadCategoryApi } from "../../../../api-service/admin";
...
function BrandCreateModal({ openModal, handleClose, refetch, modalId } : any) {
...
const getBrandData = useQuery({
  queryKey : ['getBrandData', modalId],
  queryFn : () => getBrandApi(`${modalId}`),
  enabled : !!modalId
})
...
useEffect(()=>{
    if(modalId){
      ...
      setIconImgUrl(brandData?.img_url)
    }
},[getBrandData?.data?.data?.result , setValue])
...
// submit function
    const onSubmit = async (data:any) =>{
      ...
      console.log(payload);
      if(modalId){
        const updateApi = await putBrandApi(modalId,payload)
        ...
        const uploadApi = await postBrandApi(payload)
